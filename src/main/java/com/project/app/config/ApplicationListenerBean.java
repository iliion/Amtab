...
public class ApplicationListenerBean implements
	...
	private void populateDatabase(ApplicationContext applicationContext) {
		...
		StopRepository stopRepo = applicationContext
				.getBean(StopRepository.class);
		LineRepository lineRepo = applicationContext
				.getBean(LineRepository.class);
		TimeTableRepository timetableRepo = applicationContext
				.getBean(TimeTableRepository.class);
...
//		// LOGGER.info("DescrizioneFermata: " +
//		// stopPositionArray[i].getDescrizioneFermata());
//		// LOGGER.info("LAT: " +
//		// stopPositionArray[i].getPosizioneFermata().getLat());
//		// LOGGER.info("LON: " +
//		// stopPositionArray[i].getPosizioneFermata().getLon());
//		//
...
//
//		for (int i = 0; i < lines.length; i++) {
//			DailyBusService[] dailyService = service.getDailyService(lines[i]
//					.getIdLinea());
//
//			LineJPA lineJPA = new LineJPA();
...
//				timetableJPA.setIdLine(lines[i].getIdLinea());
...
//		lineRepo.flush();
		...
		for (int i = 0; i < stops.size(); i++) {
			StopJPA stop = stops.get(i);
			stop.setStopTimetables(timetableRepo.findByIdStop(stop.getIdStop()));
			stopRepo.save(stop);
		}
